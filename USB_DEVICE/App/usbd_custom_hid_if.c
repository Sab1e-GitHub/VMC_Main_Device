...
0x05, 0x01,        // Usage Page (Generic Desktop Ctrls)
				...
				0x91, 0x02, //     Output (Data,Var,Abs,No Wrap,Linear,Preferred State,No Null Position,Non-volatile)
				//1B
				0x09, 0x75, //     Usage (0x70)		Ramp Start
				...
				0x09, 0x6E, //       Usage (0x6E)			Set Periodic Report
				0x09, 0x73, //       Usage (0x73)			Set Constant-Force Report
				0x09, 0x74, //	   Usage (0x73)			Set Ramp-Force Report
				0x75, 0x08,        //       Report Size (8)
				0x95, 0x06,        //       Report Count (6)
				0xB1, 0x02, //       Feature (Data,Var,Abs,No Wrap,Linear,Preferred State,No Null Position,Non-volatile)
				//6B
...
static int8_t CUSTOM_HID_OutEvent_FS(uint8_t event_idx, uint8_t state) {
	/* USER CODE BEGIN 6 */
	// 如果USB_Out_Flag为0，说明数据正在处理中，不能覆盖，抛弃数据。
	// 当且仅当主机发送频率超过单片机处理能力时，数据才会被抛弃。
	if (USB_Out_Flag == 0) {
...
static int8_t CustomHID_GetFeature(uint8_t event_idx, uint8_t *buffer,
	...
	case PARAMETER_BLOCK_POOLS_REPORT_ID:
		...
		data[0] = ram_size & 0xFF;		//发送低位
		data[1] = ram_size >> 8;			//发送高位
